...
Version: 0.7.1
Plugin URI: https://wordpress.org/plugins/simplesamlphp-authentication/
...
if (!class_exists('SimpleSAMLAuthenticator')) {
    class SimpleSAMLAuthenticator
    {
        ...
        * to wp-admin.
        */
        public function bypass_reauth($login_url)
        {
        ...
        simpleSAMLphp plugin!</p>'), $username, get_option('admin_email'));
            ...
            account!</p>'), $username, get_option('admin_email'));
                    $errors['registerfail'] = $error;
                    echo $error;
                    echo '<p><a href="/wp-login.php?action=logout">Log out</a> of simpleSAMLphp.</p>';
                    exit();
                }
            }
        }
